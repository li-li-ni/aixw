...
class ChatDataProcessor:
    ...
    def load_qq_export_from_content(self, content):
        messages = []
        lines = content.split('\n')
        current_talker = None
        current_content = []
...
CHAT_DATA_FILE_JSON = 'chat_data.json'
CHAT_DATA_FILE_TXT = 'chat_data.txt'

def load_chat_data():
    processor = ChatDataProcessor()
    
    if os.path.exists(CHAT_DATA_FILE_JSON):
        processor.load_from_json(CHAT_DATA_FILE_JSON)
        print(f"已加载聊天记录文件: {CHAT_DATA_FILE_JSON}")
    elif os.path.exists(CHAT_DATA_FILE_TXT):
        processor.load_from_text(CHAT_DATA_FILE_TXT)
        print(f"已加载聊天记录文件: {CHAT_DATA_FILE_TXT}")
    else:
          print(f"未找到聊天记录文件，使用示例数据")
        sample_data = [
